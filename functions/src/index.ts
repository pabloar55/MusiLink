...
function notificationPath(data: Record<string, string>): string {
...
async function sendNotification(
  ...
  await Promise.all(
    [...targets].map(async ([token, tokenRef]) => {
      try {
...
function normalizeArtistIdentityName(value: string): string {
...
export const acceptFriendRequest = onCall(
  ...
  async (request) => {
    ...
    if (
      ...
      throw new HttpsError('invalid-argument', 'Valid requestId and senderId values are required.');
    }
